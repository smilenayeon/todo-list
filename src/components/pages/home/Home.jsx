      ...
      {items.map((item) => (
        <div key={item.id} className='item-container'>
          {editItemId === item.id ? (
            ...
            <div>
              <h3 className={`item-text ${isComplete[item.id] ? 'line-through' : ''}`}>{item.text}</h3>
              <div className='item-control'>
                ...
                <div className='item-edit-button' onClick={() => handleEdit(item.id)}>
                  <i className='fa-regular fa-pen-to-square'></i>Edit
                </div>
                <div className='item-delete-button' onClick={() => handleDelete(item.id)}>
